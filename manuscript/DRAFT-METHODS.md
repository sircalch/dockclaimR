...
**Figure 1. Seed-level rank agreement in the DUD-E workflow study.** Bars show
the three pairwise Spearman correlations of ligand ranks for each target under
fixed seeds 1001, 2002, and 3003. The figure is generated by
`tools/render_multitarget_results.R` from the retained target-specific run
tables. The bars summarize agreement across complete ranked lists and must not
be interpreted as evidence of binding, activity, or screening performance.
